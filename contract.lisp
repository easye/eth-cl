  ...
  (when path
    (let* ((abbreviated-file
           ...
           (data
             (make-contract
              (format nil "~a:~a"
                      abbreviated-file class-name)
              (format nil "~a~a.json"
                      (make-pathname :directory (pathname-directory path)) (pathname-name path)))))
...
(defun make-contract (name file)
  ...
  (gethash name (gethash "contracts"
                         (with-open-file (in file) (jsown-as-hashtable (jsown:parse in))))))
